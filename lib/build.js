...
async function build (gyp, argv) {
  ...
  async function loadConfigGypi () {
    ...
    try {
      ...
      if (err.code === 'ENOENT') {
        throw new Error('You must run `nw-gyp configure` first!')
      } else {
        throw err
      }
    }
  ...
  async function doWhich (msvs) {
    // On Windows use msbuild provided by nw-gyp configure
    if (msvs) {
      if (!config.variables.msbuild_path) {
        throw new Error('MSBuild is not set, please run `nw-gyp configure`.')
      }
