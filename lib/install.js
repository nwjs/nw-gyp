...
async function install (gyp, argv) {
  ...
  const releaseDetails = {
    name: 'nw',
    headersUrl: `https://dl.nwjs.io/v${version}/nw-headers-v${version}.tar.gz`,
    libUrl: `https://dl.nwjs.io/v${version}/x64/nw.lib`
  }
  const release = processRelease(argv, gyp, version, releaseDetails)
  ...
  if (gyp.opts.ensure) {
    ...
    if (win) {
      log.verbose('on Windows; need to check nw.lib')
      const nodeLibPath = path.resolve(devDir, arch, 'nw.lib')
      try {
  ...
  async function go () {
    ...
    // Ommited on Windows if only new nw.lib is required
    ...
    const tarExtractDir = win ? await fs.mkdtemp(path.join(os.tmpdir(), 'nw-gyp-tmp-')) : devDir
    ...
    try {
        ...
        if (tarPath) {
          ...
          // something went wrong downloading the tarball?
            if (err.code === 'ENOTFOUND') {
              throw new Error('This is most likely not a problem with nw-gyp or the package itself and\n' +
      ...
      // need to download nw.lib
        ...(win ? [downloadNodeLib(), downloadNodeLib('node')] : []),
    ...
    async function downloadNodeLib (libName = undefined) {
      const releaseName = libName || release.name
      if (libName && releaseName === libName) {
        return
      }
      log.verbose('on Windows; need to download `' + releaseName + '.lib`...')
      const dir = path.resolve(tarExtractDir, arch)
      const targetLibPath = path.resolve(dir, releaseName + '.lib')
      let { libUrl, libPath } = release[arch]
      if (libName) {
        libUrl = libUrl.replace(`${release.name}.lib`, `${libName}.lib`)
        libPath = libPath.replace(`${release.name}.lib`, `${libName}.lib`)
      }
      const name = `${arch} ${releaseName}.lib`
      ...
      // Since only required nw.lib is downloaded throw error if it is not fetched
      if (res.status !== 200) {
